	...
	query := fmt.Sprintf(`
		WITH tile AS (
			SELECT
				...
				area_sq_m,
				ST_AsMVTGeom(
					ST_Simplify(geometry, CASE 
						WHEN $1 < 10 THEN 0.01
						WHEN $1 < 12 THEN 0.001
						ELSE 0.0001
					END),
					ST_TileEnvelope($1, $2, $3),
					4096,
	...
	query := `
		WITH tile AS (
			SELECT
				...
				name,
				surface,
				COALESCE(access, 'unknown') as access,
				length as width_m,
				ST_AsMVTGeom(
					geometry,
					ST_TileEnvelope($1, $2, $3),
...
func (r *environmentRepository) GetEnvironmentRadiusTile(ctx context.Context, lat, lon, radiusKm float64) ([]byte, error) {
	query := `
		...
		beaches_mvt AS (
			...
			WHERE b.geometry && circle.geom
			  AND ST_Contains(circle.geom, b.geometry)
			ORDER BY b.name
			LIMIT 20
		)
		SELECT 
			COALESCE(ST_AsMVT(green_mvt.*, 'green_spaces'), ''::bytea) ||
			COALESCE(ST_AsMVT(beaches_mvt.*, 'beaches'), ''::bytea) AS tile
		FROM green_mvt
		FULL OUTER JOIN beaches_mvt ON true
	`
	...
	if err != nil {
		r.logger.Error("Failed to generate environment radius tile",
			zap.Float64("lat", lat),
			zap.Float64("lon", lon),
